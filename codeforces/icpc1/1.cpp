// passed till 48 testcases
// https://codeforces.com/contest/2073/problem/A
// the 2.cpp file has the correct version with int128 . taking care of potential
...
void solve() {
  ...
  for (int i = 0; i < r; i++) {
    ...
    ll contrib = (s * s - q) - (row_count[i] - 2) * n * (n - 1);
    ans += contrib;
  }
  ...
  for (int j = 0; j < c; j++) {
    ...
    ll contrib = (s * s - q) - (col_count[j] - 2) * m * (m - 1);
    ans += contrib;
  }
